...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message =
        err?.code === "USER_ALREADY_EXISTS" || /already/i.test(err?.message || "")
  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 py-16">
      <div className="bg-white dark:bg-[#171a22] rounded-2xl card-shadow border border-brand-100 dark:border-white/10 p-8">
        <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white text-center">Register</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mt-1.5">Create your MediQueue account</p>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mt-6">
          ...
          <Link href="/login" className="text-brand-700 dark:text-brand-300 font-semibold hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
